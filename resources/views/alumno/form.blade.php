<div class="row padding-1 p-1">
    <div class="col-md-12">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" value="{{ old('nombre', $alumno?->nombre) }}" id="nombre" placeholder="Nombre(s)">
            {!! $errors->first('nombre', '<div class="invalid-feedback" role="alert"><strong>:message</strong></div>') !!}
        </div>
        <div class="form-group mb-2 mb20">
            <label for="apellido_pat" class="form-label">{{ __('Apellido Paterno') }}</label>
            <input type="text" name="apellido_pat" class="form-control @error('apellido_pat') is-invalid @enderror" value="{{ old('apellido_pat', $alumno?->apellido_pat) }}" id="apellido_pat" placeholder="Paterno">
        ...
        <div class="form-group mb-2 mb20">
            ...
            <input type="text" name="apellido_mat" class="form-control @error('apellido_mat') is-invalid @enderror" value="{{ old('apellido_mat', $alumno?->apellido_mat) }}" id="apellido_mat" placeholder="Materno">
        ...
        {{-- Carrera (select de FK) --}}
        <div class="form-group mb-2">
            <label for="fk_carrera" class="form-label">Carrera</label>
            <select name="fk_carrera" id="fk_carrera"
            class="form-select @error('fk_carrera') is-invalid @enderror">
            <option value="">Seleccione...</option>
                @foreach($carreras as $id => $nombre)
            <option value="{{ $id }}"
                @selected(old('fk_carrera', $alumno?->fk_carrera) == $id)>
            {{ $nombre }}
            </option>
                @endforeach
            </select>
                {!! $errors->first('fk_carrera', '<div class="invalid-feedback"><strong>:message</strong></div>') !!}
        </div>

        <div class="row" style="margin-bottom: 1%">
            <div class="col-md-3">
                <label class="form-label">Año</label>
                <input type="number" name="anio" class="form-control" value="{{ old('anio', now()->year) }}" min="2000">
            </div>
            <div class="col-md-3">
                <label class="form-label">Periodo</label>
                <select name="periodo" class="form-select">
                    <option value="1" @selected(old('periodo')==1)>Enero-Junio</option>
                    <option value="2" @selected(old('periodo')==2)>Agosto-Diciembre</option>
                </select>
            </div>
        </div>
    </div>
