                        ...
                        <form method="POST" action="{{ route('asignaciones.update', $asignacion->id) }}"  role="form" enctype="multipart/form-data">
                            ...
                            <div class="box box-info padding-1">
                                        ...
                                        <select name="profesor_id" id="profesor_id" class="form-control @error('profesor_id') is-invalid @enderror">
                                            ...
                                            @foreach ($profesores as $profesor)
                                                <option value="{{ $profesor->id_profesor }}" 
                                                    {{ (old('profesor_id') == $profesor->id_profesor || $asignacion->profesor_id == $profesor->id_profesor) ? 'selected' : '' }}>
                                        ...
                                        <select name="materia_id" id="materia_id" class="form-control @error('materia_id') is-invalid @enderror">
                                            ...
                                            @foreach ($materias as $materia)
                                                <option value="{{ $materia->id_materia }}" 
                                                    {{ (old('materia_id') == $materia->id_materia || $asignacion->materia_id == $materia->id_materia) ? 'selected' : '' }}>
                                                    {{ $materia->nombre_mat }}
                                                </option>
                                            @endforeach
                                        </select>
                                        ...
                                        <select name="aula_id" id="aula_id" class="form-control @error('aula_id') is-invalid @enderror">
                                            ...
                                            @foreach ($aulas as $aula)
                                                <option value="{{ $aula->id }}" 
                                                    {{ (old('aula_id') == $aula->id || $asignacion->aula_id == $aula->id) ? 'selected' : '' }}>
                                                    {{ $aula->salon }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <select name="dia_semana" id="dia_semana" class="form-control @error('dia_semana') is-invalid @enderror">
                                            ...
                                            @foreach (['Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado', 'Domingo'] as $dia)
                                                <option value="{{ $dia }}" 
                                                    {{ (old('dia_semana') == $dia || $asignacion->dia_semana == $dia) ? 'selected' : '' }}>
                                                    {{ $dia }}
                                                </option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group mb-3">
                                        ...
                                        <select name="hora_inicio" id="hora_inicio" class="form-control @error('hora_inicio') is-invalid @enderror @error('horario') is-invalid @enderror">
                                            ...
                                            @foreach ($horariosDisponibles as $key => $label)
                                                <option value="{{ $key }}" 
                                                    {{ (old('hora_inicio') == $key || $asignacion->hora_inicio == $key) ? 'selected' : '' }}>
                                                    {{ $label }}
                                                </option>
                                            @endforeach
                                        </select>
                                ...
                                <div class="box-footer mt-4">
                                    <button type="submit" class="btn btn-primary">{{ __('Actualizar Asignación') }}</button>
                                </div>
                            </div>
